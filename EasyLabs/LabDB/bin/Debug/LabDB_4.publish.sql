...
GO
/*
The column [dbo].[User].[hash] is being dropped, data loss could occur.

The column [dbo].[User].[passhash] on table [dbo].[User] must be added, but the column has no default value and does not allow NULL values. If the table contains data, the ALTER script will not work. To avoid this issue you must either: add a default value to the column, mark it as allowing NULL values, or enable the generation of smart-defaults as a deployment option.
*/
...
GO
ALTER TABLE [dbo].[Client] DROP CONSTRAINT [FK__Client__user_id__4D94879B];
...
GO
ALTER TABLE [dbo].[Employee] DROP CONSTRAINT [FK__Employee__user_i__5070F446];
...
IF EXISTS (SELECT TOP 1 1 
    ...
    BEGIN
        ...
        INSERT INTO [dbo].[tmp_ms_xx_User] ([user_id], [email], [salt])
        SELECT   [user_id],
                 [email],
                 [salt]
        FROM     [dbo].[User]
...
AS
Begin
	insert into dbo.[User] (email,salt,passhash) 
...
DECLARE tableconstraintnames CURSOR LOCAL FORWARD_ONLY
    FOR SELECT SCHEMA_NAME([schema_id]),
               ...
               0
        FROM   [sys].[objects]
        WHERE  [parent_object_id] IN (OBJECT_ID(N'dbo.Client'), OBJECT_ID(N'dbo.Employee'))
               AND [type] IN (N'F', N'C')
